...
6. [TCR–pMHC Interaction Prediction](#6-tcrpmhc-interaction-prediction)
...
| **Conformational B-cell epitope** | Discontinuous residues brought together by 3D folding, recognized by antibodies | **Hard** (AUC typically 0.58–0.80) |
| **T-cell epitope (MHC-I)** | 8–11mer peptide bound to HLA class I, recognized by CD8+ T cells | Relatively mature (AUC 0.92–0.98) |
| **T-cell epitope (MHC-II)** | 13–25mer peptide bound to HLA class II, recognized by CD4+ T cells | Moderate (AUC 0.82–0.90) |
...
- **pMHC** — peptide–MHC complex; the ligand recognized by T-cell receptors
...
| **2015–2019** | Shallow ML: SVM, Random Forest, early ANNs | NetMHCpan 3.0/4.0, MHCflurry 1.0, BepiPred-2.0 |
| **2020–2022** | Deep neural networks, protein language models emerge | NetMHCpan 4.1, MHCflurry 2.0, BepiPred-3.0, DeepImmuno |
| **2023–2025 (current)** | Transformers + GNNs + structural data, hybrid architectures | MUNIS, BigMHC, GraphEPN, DiscoTope 3.0, EpiGraph |
...
- **B-cell epitope prediction (conformational):** Active frontier; best tools achieve AUC–PR ≈0.23–0.25, reflecting the inherent difficulty of the problem.
- **TCR–pMHC prediction:** Exploding new subfield; 30+ methods in 2024–2025 alone. Generalization to unseen epitopes remains poor.
...
### Top Venues Publishing in This Area (2024–2025)
...
|-----|------|--------------|---------|
| **ESM-2 (650M–15B params)** | Up to 15B | UniRef90 | MUNIS, BepiPred-3.0, EpiGraph, EpitopeTransfer, TransHLA |
...
**Key insight from EpitopeTransfer (2025):** Fine-tuning PLMs on phylogenetically related epitope data before downstream classification significantly outperforms using frozen embeddings or general fine-tuning (AUC gains of +0.09–0.13 over BepiPred-3.0, EpiDope, EpitopeVec, and epitope1D).
...
| **EpiGraph (2024)** | ESM-2 + ESM-IF1 embeddings → surface graph | GAT with residual connections | AUC–PR 0.24 vs 0.20 BepiPred-3.0 |
...
The strongest 2024–2025 models increasingly combine multiple paradigms:
...
- **UniPMT** — Heterogeneous GNN with multi-task learning — simultaneously predicts P–M–T, P–M, and P–T binding within one framework

### 3.5 Traditional Machine Learning (Still Relevant)

Despite the deep learning revolution, traditional ML retains niches:

- **SVM/Random Forest:** TCR-H (2024) uses SVM with physicochemical features, achieving strong generalization (AUC 0.87–0.89 on strict epitope splitting)
...
| **MHCflurry 2.0** | 2020 | ANN ensemble | IEDB + MS | ~0.92–0.93 | Yes (GitHub) |
...
| **BepiPred-2.0** | 2019 | Random Forest (sequence features) | AUC ~0.58–0.62 |
...
| Tool | Year | Approach | AUC | AUC–PR | Code |
|------|------|----------|-----|--------|------|
...
| **SEMA 2.0** | 2024 | Ensemble (sequence + 3D) | 0.76–0.80 | — | Yes |
| **BepiPred-3.0** | 2022 | ESM-2 + ANN | ~0.70–0.71 | 0.177 | Yes (web server) |
| **epitope3D** | 2022 | XGBoost + graph features | — | Competitive | Yes |
| **EpiDope** | 2021 | BiLSTM | ~0.67 | — | Yes |

**Important caveat:** AUC–PR values in the 0.20–0.25 range appear low, but they reflect the genuine difficulty of conformational epitope prediction on highly imbalanced benchmark sets where epitope residues are <10% of surface residues. The field trend is positive (0.177 BepiPred-3.0 → 0.24 EpiGraph → GraphEPN improvements).
...
|------|------|---------------|
| **WALLE (NeurIPS 2024)** | 2024 | 3–10× improvement over general binding-site methods; PLM + bipartite GNN |
...
| **DeepVacPred** | 2021–2023 | End-to-end from epitope selection to multi-epitope vaccine construct design |
...
| **IEDB Benchmark** | Curated MHC binding benchmarks (BD2009, BD2013) | 135K–176K data points | tools.iedb.org/benchmark |
...
| **AsEP (NeurIPS 2024)** | 2024 | Antibody-specific epitope prediction | Clustered epitope groups; WALLE 3–10× better |
...
## 6. TCR–pMHC Interaction Prediction
...
- **Qi et al. (2025)** — *"A Roadmap for TCR–pMHC Binding Prediction"* — *Briefings in Bioinformatics* — Most comprehensive survey; compared 22 pMHC + 30 TCR specificity methods
...
| **TCR-H** | 2024 | SVM + physicochemical features + SHAP | Strong generalization to unseen epitopes | AUC 0.87–0.89 |
...
3. **CDR3-only limitation:** Methods using full TCR sequence (α+β chain) outperform CDR3-only but are data-limited
...
### 8.3 Current Clinical Trials (2024–2025)
...
| 3 | ELISPOT / FluoroSpot (IFN-γ) | Medium | High (functional T-cell response) |
...
2. **Conformational B-cell epitopes**: Despite progress with GNNs and structural data, AUC–PR values remain at 0.23–0.25. This is partly an intrinsic difficulty problem (epitope residues are <10% of surface) and partly a data problem (<250 confirmed conformational epitopes in CEDb).
...
- **Why read:** Deepest survey of TCR–pMHC prediction; benchmarks 22 pMHC + 30 TCR methods; proposes a structured roadmap.
...
- **Why read:** Demonstrates that current MHC binding predictors fail on structurally valid novel peptides (AUROCs 0.06–0.22); structure-guided generative design.
...
**8. Rajeshwar et al. (2024)** — *"TCR-H: Explainable machine learning prediction of TCR–pMHC binding"*
...
- *Scientific Reports*
- **Why read:** State-of-the-art conformational epitope prediction; GAT with PLM embeddings; AUC–PR 0.24.
...
- **Coverage period:** Emphasis on 2023–2026, with key earlier papers included where foundational
- **Search terms:** "deep learning epitope prediction," "B-cell/T-cell epitope," "MHC binding prediction," "TCR–pMHC," "conformational epitope GNN," "protein language model epitope," "neoantigen prediction," "generative epitope design"
