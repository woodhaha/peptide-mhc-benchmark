#!/usr/bin/env Rscript
# Autoresearch Evaluator: IEDB Benchmark with Specificity ≥ 85% & Sensitivity ≥ 90%
# Usage: Rscript evaluator.R <model_path.h5>
# Output: JSON line with {pass, specificity, sensitivity, f1, accuracy, tp, fp, tn, fn}
...
if (length(args) < 1) {
  cat('{"error":"Missing model path argument. Usage: Rscript evaluator.R <model.h5>"}\n')
  quit(status = 1)
}
model_path <- args[1]
...
# ---- Output JSON ----
json_output <- sprintf(
  '{"pass":%s,"specificity":%.4f,"sensitivity":%.4f,"f1":%.4f,"accuracy":%.4f,"tp":%d,"fp":%d,"tn":%d,"fn":%d,"n_total":%d,"model":"%s"}',
  ifelse(passed, "true","false"), specificity, sensitivity, f1, accuracy,
  tp, fp, tn, fn, nrow(results), basename(model_path)
)
cat(json_output, "\n")
